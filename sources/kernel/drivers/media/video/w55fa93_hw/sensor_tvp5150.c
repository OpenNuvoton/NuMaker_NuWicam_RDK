...
static s8  DrvVideoIn_I2cWriteTVP5150(__u8 uAddr, __u16 uRegAddr, __u8 uData)
{
	...
	buffer[0] = (u8)(uRegAddr&0xFF);
	buffer[1] = uData;
//         printk("tvp5150: writing 0x%02x 0x%02x\n", buffer[0], buffer[1]);
	if (2 != (rc = i2c_master_send(save_client, buffer, 2)))
	{
...
static __s8  DrvVideoIn_I2cReadTVP5150(__u8 uAddr, __u16 uRegAddr)
{
    ...
    struct i2c_msg msgs;
    int ret=-1;
    u8 buf = (u8)(uRegAddr&0xff);
	int rc=0;
	unsigned char buffer[1];
	
	struct i2c_msg msg[] = {
		{ .addr = save_client->addr, .flags = 0,	.buf = &buf, 	.len = 1 },
		{ .addr = save_client->addr, .flags = I2C_M_RD, .buf = buffer, 	.len = 1 }
	};

	rc = i2c_transfer(save_client->adapter, msg, 2);
	if (rc < 0 || rc != 2) {
...
void TVP5150SetVideoformat(int std)
{
	...
	__u8 u8Data = 0;
	do {
		ret = DrvVideoIn_I2cWriteTVP5150(uDeviceID, 0x7F, 0x00);
		u8Data = DrvVideoIn_I2cReadTVP5150(uDeviceID, 0x7F);
		printk("Wayne: [0x%02X] -> Value=[0x%02X, 0x%02X]\n", 0x7F, 0x00, u8Data );
		if ( u8Data == 0x00 ) break;			
	} while(1);
	if( ret < 0 )
		printk("Dannel: Programminmg TVP5150 REG 0x7F, 0x0 fail\n");
	else
		printk("Dannel: Programminmg TVP5150 REG 0x7F, 0x0 successful\n");
#endif
	
	uDeviceID  = g_uTVP5150DeviceID[TVP5150_Initial];	
	/* First tests should be against specific std */
	if (std == TVP_STD_ALL) {
		fmt=0x0;	/* Autodetect mode */
		...
		fmt=0x8;
	} else if (std & TVP_STD_NTSC) { 	/* Then, test against generic ones */
		fmt=0x2;
	...
	do {
		...
		u8Data = DrvVideoIn_I2cReadTVP5150(uDeviceID, TVP5150_VIDEO_STD);
		printk("Wayne: [0x%02X] -> Value=[0x%02X, 0x%02X]\n", TVP5150_VIDEO_STD, fmt, u8Data );
		if ( u8Data == fmt ) break;			
	} while(1);
	if( ret < 0 )
		printk("Dannel: Programminmg TVP5150 REG 0x%02X fail\n", TVP5150_VIDEO_STD);
	else
		printk("Dannel: Programminmg TVP5150 REG 0x%02X ok\n", TVP5150_VIDEO_STD);
		
#if 1		
	printk("Dannel Suggestion\n");		
	do {
		ret = DrvVideoIn_I2cWriteTVP5150(uDeviceID, 0x01, 0x14);
		u8Data = DrvVideoIn_I2cReadTVP5150(uDeviceID, 0x01);
		printk("Wayne: [0x%02X] -> Value=[0x%02X, 0x%02X]\n", 0x01, 0x14, u8Data );
		if ( u8Data == 0x14 ) break;			
	} while(1);
	
	do {
		printk("Dannel: TVP5150 V/H unlock\n");
		fmt = DrvVideoIn_I2cReadTVP5150(uDeviceID, 0x88);
	} while( (fmt & 0x06) != 0x06 );
	printk("Dannel: TVP5150 V/H locked\n");
	
	do {
		ret = DrvVideoIn_I2cWriteTVP5150(uDeviceID, 0x03, 0x69);
		u8Data = DrvVideoIn_I2cReadTVP5150(uDeviceID, 0x03);
		printk("Wayne: [0x%02X] -> Value=[0x%02X, 0x%02X]\n", 0x03, 0x69, u8Data );
		if ( u8Data == 0x69 ) break;			
	} while(1);
#endif			
...
void TVP5150SetInputSource(int src)
{
...
#if 1	
	{
		...
		u8 u8Data=0;
		do {
			ret = DrvVideoIn_I2cWriteTVP5150(uDeviceID, TVP5150_VD_IN_SRC_SEL_1, inputsrc);
			u8Data = DrvVideoIn_I2cReadTVP5150(uDeviceID, TVP5150_VD_IN_SRC_SEL_1);
			printk("Wayne: [0x%02X] -> Value=[0x%02X, 0x%02X]\n", TVP5150_VD_IN_SRC_SEL_1, inputsrc, u8Data );
			if ( u8Data == inputsrc ) break;			
		} while(1);
	}
#endif		
}
...
void TVP5150SetResolution(int index)
{
	...
	for(i=0;i<uTableSize; i++, psRegValue++)
	{
		...
		while (1) {
			...
			if ( psRegValue->uRegAddr == 0xC0   ) break;
			u8Data = DrvVideoIn_I2cReadTVP5150(uDeviceID, (psRegValue->uRegAddr));
			printk("Wayne: ITEM:%d [0x%02X] -> Value=[0x%02X, 0x%02X]\n", i, psRegValue->uRegAddr, psRegValue->uValue, u8Data );
			if ( u8Data == (psRegValue->uValue) ) break;			
		}		
	}
}
