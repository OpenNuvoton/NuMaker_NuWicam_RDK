...
static void tcp_conn_flush(void *d)
{
...
#else
	int iAvailBytes = avail_send_buf(c);

	if (iAvailBytes < len + 4) {
		tcp_conn_flush(c);

		iAvailBytes = avail_send_buf(c);
...
static int handle_TEARDOWN(struct req *req)
{
	...
	spook_log(SL_INFO, "tearing down %s in session %s",
