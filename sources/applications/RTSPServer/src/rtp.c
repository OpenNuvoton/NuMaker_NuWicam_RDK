...
struct rtp_endpoint *new_rtp_endpoint(int payload)
{
	...
	gettimeofday(&ep->last_rtcp_send, NULL);
	ep->trans_type = 0;
...
static int rtcp_send(struct rtp_endpoint *ep)
{
	...
	if (ep->trans_type != RTP_TRANS_INTER) {
		if (now.tv_sec - ep->last_rtcp_recv.tv_sec > 20) {
			spook_log(SL_INFO, "client timeout (no RTCP received in 20 seconds)");
			ep->session->teardown(ep->session, ep);
			return -1;
		}
	}
					...
					unsigned int timestamp, int marker)
{
	...
	if (ep->force_rtcp) {
		...
		if (now.tv_sec - ep->last_rtcp_send.tv_sec > 10) {
			S_RATE_CTRL_ACT sAction;
			spook_log(SL_INFO, "server timeout, try to decrease bitrate(no RTCP send in 10 seconds)");

			sAction.eActType = eRATE_CTRL_ACT_DEC_BITRATE;
			sAction.i32Value = 50;
			QOSAnalyser_SendSuggActCommand(ep, &sAction);
