...
static void convert_uyvy_to_rgb24(struct frame *uyvy, void *d)
{
	...
	if (rgb == NULL) {
		spook_log(SL_ERR, "convert_uyvy_to_rgb24() unable to get frame %s");
		return;
	}
...
struct stream *new_stream(char *name, int format, void *d)
{
	...
	s->set_running = NULL;
	s->new_listener_notify = NULL;		//Added by chchen59
	s->get_enc_pipe_num = NULL;			//Added by chchen59
	s->private = d;
	return s;
}
...
void new_listener_notify(struct stream_destination *dest)		//Added by chchen59
{
	struct stream *s = dest->stream;

	if (s->new_listener_notify)
		s->new_listener_notify(s);
}

void get_enc_pipe_num(struct stream_destination *dest, int *pipe_num)		//Added by chchen59
{
	struct stream *s = dest->stream;

	if (s->get_enc_pipe_num)
		s->get_enc_pipe_num(s, pipe_num);
}
