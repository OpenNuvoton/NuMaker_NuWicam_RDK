	...
	while (c->send_buf_r != c->send_buf_w) {
		...
		if (ret <= 0) {
			if (ret < 0 && errno == EAGAIN) {
				//				printf("DDDDDDDDDDD conn_write return  <=0 ret %d len %d DDDDDDDDDDDDD \n", ret, len);
				continue;
			}
...
static int handle_QT_tunnel(struct req *req, int post)
{
	...
	if (!(t = get_header(req->req, "x-sessioncookie")) ||
		...
		send_data(req->conn, rep, sprintf((char *)rep,
										  "HTTP/1.0 400 Missing session cookie\r\nConnection: close\r\nContent-Type: text/html\r\n\r\n<html><body><p>Broken RTSP/HTTP tunnel request</p></body></html>\n"));
	...
	if (! cg) {
		...
		send_data(req->conn, rep, sprintf((char *)rep,
										  "HTTP/1.0 400 Unknown session cookie\r\nConnection: close\r\nContent-Type: text/html\r\n\r\n<html><body><p>Broken RTSP/HTTP tunnel request</p></body></html>\n"));
		req->conn->drop_after = 1;
		return 0;
	}
	...
	case CONN_PROTO_START:
		if ((a = strchr((char *)c->req_buf, '\n'))) {
			*a = 0;
			...
			if (!(b = strrchr((char *)c->req_buf, ' '))) return -1;
...
static void do_accept(struct event_info *ei, void *d)
{
	...
	if ((fd = accept(listener->fd, (struct sockaddr *)&addr, (socklen_t *)&i)) < 0) {
