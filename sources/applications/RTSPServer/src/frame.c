...
// #define METER_DEBUG
#define __NVT_FRAME__
#define __EXCHANGER_READ_ONCE__
...
struct frame *enlarge_frame_buffer(struct frame *f, int new_size)
{
	if (new_size == 0) {
		return NULL;
	}
	...
	new_frame = (struct frame *)realloc(f, sizeof(struct frame) + new_size);

	if (new_frame) {
		new_frame->size = new_size;
		new_frame->d = (unsigned char *)new_frame + sizeof(struct frame);
...
int get_max_frame_slot(void)
{
	return max_frame_slot;
}
...
static void exchanger_read(struct event_info *ei, void *d)
{
...
#if !defined(__EXCHANGER_READ_ONCE__)
	...
	for (;;) {
		...
		f = ex->master_read->f;
#if defined(__NVT_FRAME__)

		if (f == NULL) {
			spook_log(SL_ERR, "We lost a frame!");
			ex->master_read->f = NULL;
			ex->master_read = ex->slave_cur;
...
int deliver_frame(struct frame_exchanger *ex, struct frame *f)
{
	unsigned char c = 0;
	...
	if (ex->slave_cur->next == ex->master_read) {
		pthread_mutex_unlock(&ex->mutex);
		unref_frame(f);
		return -1;
	}
