...
struct live_source {
	...
	int i32MaxRTPTransSize;
	unsigned int u32MaxConnNum;
	unsigned int u32CurConn;
};
...
static int live_setup(struct session *s, int t)
{
	...
	s->ep[t]->max_data_size = ls->source->i32MaxRTPTransSize;
	s->ep[t]->pvQosAnalyser = SimpleQOSAnalyserNew(s->ep[t]);
	s->ep[t]->i32Format = ls->source->track[t].stream->stream->format;
	get_enc_pipe_num(ls->source->track[t].stream, &s->ep[t]->i32VideoEncPipe);
...
static void live_play(struct session *s, double *start)
{
	...
	for (t = 0; t < MAX_TRACKS && ls->source->track[t].rtp; ++t)
		if (s->ep[t]) {
			...
			new_listener_notify(ls->source->track[t].stream);	//Added by chchen59
		}
}
...
static void live_teardown(struct session *s, struct rtp_endpoint *ep)
{
	...
	if (remaining == 0) {
		if ((source->u32CurConn > 0) && (ls->playing == 1))
			source->u32CurConn--;
...
static struct session *live_open(char *path, void *d)
{
	...
	if (source->u32CurConn >= source->u32MaxConnNum)
		return NULL;
...
static void *start_block(void)
{
	...
	source->u32MaxConnNum = 0xFFFFFFFF;
	source->u32CurConn = 0;
	...
	source->i32MaxRTPTransSize = 1400;
...
static int set_track(int num_tokens, struct token *tokens, void *d)
{
	...
	int t, formats[] = { FORMAT_MPEG4, FORMAT_MPV, FORMAT_H263, FORMAT_JPEG,
						 FORMAT_PCM, FORMAT_ALAW, FORMAT_MPA, FORMAT_H264,
						 FORMAT_ADPCM, FORMAT_ULAW, FORMAT_AAC, FORMAT_G726
	...
	if (!(source->track[t].stream = connect_to_stream(tokens[1].v.str,
									next_live_frame, &source->track[t], formats, 12))) {  //TODO: Add new format, 12: count of formats[]
		spook_log(SL_ERR,
...
//								   source->track[t].stream->stream);
	...
	case FORMAT_ADPCM:
//		source->track[t].rtp = new_rtp_media_adpcm_stream(
//								   source->track[t].stream->stream);
		break;

	case FORMAT_AAC:
//		source->track[t].rtp = new_rtp_media_aac_stream(
//								   source->track[t].stream->stream);
		break;

	case FORMAT_G726:
//		source->track[t].rtp = new_rtp_media_g726_stream(
//								   source->track[t].stream->stream);
		break;
	}
...
static int set_mtu(int num_tokens, struct token *tokens, void *d)
{
	struct live_source *source = (struct live_source *)d;

	source->i32MaxRTPTransSize = tokens[1].v.num;

	return 0;
}

static int set_conn(int num_tokens, struct token *tokens, void *d)
{
	struct live_source *source = (struct live_source *)d;

	source->u32MaxConnNum = tokens[1].v.num;
...
static struct statement config_statements[] = {
	...
	{ "mtu", set_mtu, 1, 1, { TOKEN_NUM } },
	{ "conn", set_conn, 1, 1, { TOKEN_NUM } },
